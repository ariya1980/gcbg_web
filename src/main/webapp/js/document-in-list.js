...
function format(d) {
	...
	for (var i = 0; i < tran.length; i++) {
		html += '<tr class="pointer" style="background-color:#ddd;" onclick="childRowClick(\'VIEW\',\'' + tran[i]._id + '\' , \''+formType+'\')">'
			...
			+ '<td><span style="display:none;">'+ moment(tran[i].effectiveDate).format("YYYYMMDD") + '</span>'+ moment(tran[i].effectiveDate).format("DD/MM/YYYY") + '</td>' 
			+ '<td><span style="display:none;">'+ moment(tran[i].expiredDate).format("YYYYMMDD") + '</span>'+ moment(tran[i].expiredDate).format("DD/MM/YYYY") + '</td>'
			+ '<td><span style="display:none;">'+ moment(tran[i].feeDueDate).format("YYYYMMDD") + '</span>'+ moment(tran[i].feeDueDate).format("DD/MM/YYYY") + '</td>' 
			+ '<td></td>' 
			+ '<td><span style="display:none;">'+ moment(tran[i].defualtBankActDate).format("YYYYMMDD") + '</span>'+ moment(tran[i].defualtBankActDate).format("DD/MM/YYYY") + '</td>' 
			+ '</tr>'
	}
	html += '</table>';
	return html;
